...
def force_register_special_tokens(tokenizer, tokens):
    print("⚙️ Forcing special token registration for:", tokens)
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            elif "qwen" in model_name.lower() or 'llava_lora4debug' in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path)
    
                special_tokens = ["<image>", "<|im_start|>", "<|im_end|>", "<coord>", "<ground>","<|endoftext|>"]

                force_register_special_tokens(tokenizer, special_tokens)
                
                from llava.model.language_model.llava_qwen import LlavaQwenConfig
                #overwrite_config = None

                if overwrite_config is not None:
                    lora_cfg = LlavaQwenConfig.from_pretrained(model_path)
                    base_cfg = LlavaQwenConfig.from_pretrained(model_base)

                    rank0_print(f"Overwriting config with {overwrite_config}")

                    for k, v in lora_cfg.to_dict().items():
                    ...
                    model_args = {
                        'embedding_size': 3584, # 3584
                        'mode' : 'eval', 
                    }
                    model_args = Namespace(**model_args)
                    model = LlavaQwenForCausalLM.from_pretrained(model_path, model_args,low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=base_cfg, **kwargs)
            ...
            model.load_state_dict(new_static, strict=False)
