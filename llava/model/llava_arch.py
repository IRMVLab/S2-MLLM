...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(self.config, 'world_position_embedding_type'):
            from llava.model.position_encoding import PositionEmbeddingSine3D, PositionEmbeddingMLP #,PositionEmbeddingROPE
            ...
            if "sample9" in self.config.world_position_embedding_type:
                ...
                n_points = 1
            self.world_position_embedding1 = PositionEmbeddingSine3D(config.hidden_size, n_points=n_points)
            self.world_position_embedding2 = PositionEmbeddingMLP(config.hidden_size, n_points=n_points)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_stattention(self):
        ...
        self.st_attention_block = self.st_attention_block.to('cuda')

    def encode_images(self,st_attention_block,images, world_coords=None):
        image_features = self.get_model().get_vision_tower()(images)
        B = 1
        num_frames, num_tokens, num_dim = image_features.shape
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            image_features = st_attention_block(image_features.reshape(B,-1,num_dim),B = B,T = num_frames, W = 27)
            image_features = image_features[:,1:,:].reshape(num_frames, num_tokens, num_dim)
    ...
    def build_voxel_to_patch_map(self,image_features, world_coords, voxel_size, voxel_origin):
        voxel_size = 0.1
        images_features_ts = torch.stack(image_features, dim=0).squeeze(0)
        N, P, D = images_features_ts.shape 
        M = N * P

        all_features = images_features_ts.reshape(M, D)           # [M, D]
        all_coords = torch.stack(world_coords, dim=0).squeeze(0).reshape(M, 3)# [M, 3]
        voxel_coords = ((all_coords - voxel_origin) / voxel_size).floor().long()  # [M, 3]
        ...
        voxel_patch_map = defaultdict(list)
        for i in range(inverse_indices.shape[0]):
            voxel_idx = inverse_indices[i].item()
            voxel_patch_map[voxel_idx].append(all_features[i])

        
        return unique_voxel_coords, voxel_patch_map
    
    def compute_voxel_grid_params(self,coords_3d, voxel_size=0.1, margin=0.05):
        coords_3d = torch.stack(coords_3d, dim=0).squeeze(0) 
        coords = coords_3d.reshape(-1, 3)  
        min_xyz = coords.min(dim=0).values - margin
        max_xyz = coords.max(dim=0).values + margin
        voxel_dims = ((max_xyz - min_xyz) / voxel_size).ceil().long()
        return voxel_dims,min_xyz
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if use_object_proposals:
            ...
            for l in range(obj_num):
                ...
                if "patch27" in self.config.object_feature_type:
                    world_coords_new = world_coords[:, :378, :378, :].reshape(-1, 14, 27, 14, 27, 3).transpose(2, 3).flatten(3, 4)  # [16, 14, 14, 27*27, 3]
                    cur_object_patch = torch.all((min_xyz <= world_coords_new) & (world_coords_new <= max_xyz), dim=-1)     # [16, 14, 14, 27*27]
                    ...
                    world_coords_new = world_coords[:, :378, :378, :].reshape(-1, 27, 14, 27, 14, 3).transpose(2, 3).flatten(3, 4)  # [16, 14, 14, 27*27, 3]
                    cur_object_patch = torch.all((min_xyz <= world_coords_new) & (world_coords_new <= max_xyz), dim=-1)     # [16, 14, 14, 27*27]
        ...
        use_hwd = False
        use_nerf_pe = False
        use_nerf_3d_pe = False
        use_multi_pe = True
        if past_key_values is None:
            B = input_ids.shape[0]
            world_coords = video_dict['world_coords']
            dir_patch = video_dict['dir_patch']
            xyz_min = world_coords.view(B, -1, 3).min(dim=1)[0]
            if len(video_dict['box_input']):
                box_input = video_dict['box_input'] 
            else:
                box_input = None
            n_points = 1
            
            world_coords_cp = world_coords
            world_coords = [self.average_coordinate_in_patch(coords, patch_size = 14) for coords in world_coords]
            world_coords_consis = [self.average_coordinate_in_patch(coords) for coords in world_coords_cp]
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            return input_ids, position_ids, attention_mask, past_key_values, None, labels, None, None,None,None,None,None,None
        ...
        if type(images) is list or images.ndim == 5:
            ...
            encoded_image_features = self.encode_images(self.st_attention_block,concat_images)
            encoded_feats = [encoded_image_features[i].unsqueeze(0) for i in range(encoded_image_features.shape[0])]
                
            encoded_image_features = torch.split(encoded_image_features, split_sizes)
            trainable_image_features = encoded_image_features
            new_image_features = []
            for idx, image_feat in enumerate(encoded_image_features):
                coords = world_coords_discrete[idx].flatten(1, 2)
                image_feat = image_feat + self.get_model().world_position_embedding1(coords.detach())
                
                new_image_features.append(image_feat)
            encoded_image_features = new_image_features
            ...
            _,_, true_height,true_width = image.shape
            patch_size = 14
            num_patches_h = true_height // patch_size
            num_patches_w = true_width // patch_size
            encoded_pos = self.position_getter(image.shape[0], num_patches_h, num_patches_w, image.device)
            encoded_pos = [encoded_pos[i].unsqueeze(0) for i in range(encoded_pos.shape[0])]
            shapes = torch.tensor([true_height, true_width]).repeat(16, 1)
            ...
            margin = self.config.voxel_margin

            
            _, voxel_origin_consis = self.compute_voxel_grid_params(world_coords_consis, voxel_size, margin)
            new_image_features = []
            _, voxel_to_patches= self.build_voxel_to_patch_map(image_features,world_coords_consis,voxel_size, voxel_origin_consis)
            for idx, image_feat in enumerate(image_features):
                image_feat = image_feat +  5 * self.get_model().world_position_embedding2(dir_patch.squeeze(0).detach(),self.mlp_pe)
                new_image_features.append(image_feat)
            ...
            image_features = self.encode_images(st_attention,images)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if hasattr(self.config, "coord_token_ids"):
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, object_features, object_boxes,voxel_to_patches,encoded_feats,encoded_pos,shapes,trainable_image_features
