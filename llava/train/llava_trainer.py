...
class LLaVATrainer(Trainer):
    ...
    def save_my_lora_ckpt(self, output_dir, args, model):
        ...
        if hasattr(self, "deepspeed") and self.deepspeed is not None:
            print("Saving DeepSpeed state at", output_dir)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        language_loss, gd_loss, regr_loss, scores = model(**inputs)
        self.regr_loss = regr_loss
        self.gd_loss = gd_loss
        self.language_loss = language_loss
        self.loss = self.gd_loss + 0.3*self.regr_loss + self.language_loss
        return self.loss
    ...
    def log(self, logs):
        ...
        if loss_value is not None:
            ...
            logs["gd_loss"] = self.gd_loss.item()
            logs["regr_loss"] = self.regr_loss.item()
            logs["language_loss"] = self.language_loss.item()
        super().log(logs)
