...
class ModelArguments:
    ...
    embedding_size: Optional[int] = field(default=None)  # 3584
...
class DataArguments:
    ...
    voxel_margin: Optional[float] = field(default=0.05)
    frame_sampling_strategy: str = 'uniform' # uniform, mc32, mc24
    dataset_list: Optional[str] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    mm_new_attn_lr: Optional[float] = None
    mm_encoder_to_decoder_proj_lr: Optional[float] = None
    mm_mlp_pe_lr: Optional[float] = None
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ["mm_projector", "vision_resampler",'fast3r','st_attention_block','mlp_pe']
    ...
    if "fast3r" in lora_module_names:
        lora_module_names.remove("fast3r")
    if "st_attention_block" in lora_module_names:
        lora_module_names.remove("st_attention_block")
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    nl_tokens = tokenizer("\n").input_ids
    if "<ground>" not in tokenizer.get_vocab():
        tokenizer.add_special_tokens({"additional_special_tokens": ["<ground>"]})
    ground_token_index = tokenizer.convert_tokens_to_ids("<ground>")
    #print(ground_token_index)
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        self.video_processor = VideoProcessor(
            ...
            dataset_list = getattr(data_args, "dataset_list", None),
        )
    ...
    def get_indices_by_dataset(self, dataset_name):
        return [i for i, n in enumerate(self.list_data_dict) if n['metadata']['dataset'].lower() == dataset_name]
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            # SEGA
            if sample.get("metadata", {}).get("dataset", None) in ["scanrefer", "multi3drefer",'nr3d','sr3d']:
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            try:
                if video_file.startswith('scannet'):
                    #print(' process_3d_video ')
                ...
                camera_intrinsics = video_dict.pop("camera_intrinsics")
                depths = video_dict.pop("depths")
                camera_pose = video_dict.pop("camera_pose")
        ...
        views = {}
        if self.list_data_dict[i].get("metadata", {}).get("dataset", None) in ["scanrefer",'nr3d','sr3d']:
            ...
            if isinstance(camera_intrinsics, torch.Tensor):
                views["camera_intrinsics"] = camera_intrinsics.to(dtype=depths.dtype, device=depths.device)
            else:
                views["camera_intrinsics"] = torch.tensor(camera_intrinsics, dtype=depths.dtype, device=depths.device)

            views["depths"] = depths
            views["camera_pose"] = camera_pose
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "views" in instances[0]:
            batch["views"] = [instance["views"]  for instance in instances]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.world_position_embedding_type is not None:
        ...
        overwrite_config["voxel_margin"] = model_args.voxel_margin
        """
            ============== x_min ================
...
def patched_load_from_checkpoint(self, resume_from_checkpoint, model=None):
    print(f"Loading only model from {resume_from_checkpoint}...")

    if model is None:
        model = self.model

    if os.path.exists(os.path.join(resume_from_checkpoint, "pytorch_model.bin")):
        model.load_state_dict(torch.load(os.path.join(resume_from_checkpoint, "pytorch_model.bin")))
    elif os.path.exists(os.path.join(resume_from_checkpoint, "adapter_model.bin")):
        from peft import PeftModel
        model = PeftModel.from_pretrained(model, resume_from_checkpoint)
        self.model = model

    trainer_state_file = os.path.join(resume_from_checkpoint, "trainer_state.json")
    if os.path.exists(trainer_state_file):
        self.state = TrainerState.load_from_json(trainer_state_file)
        print(f"Loaded trainer_state: step={self.state.global_step}, epoch={self.state.epoch}")
    else:
        print(" trainer_state.json not found; starting from scratch")
...
def train(attn_implementation=None):
    ...
    model_args.voxel_margin = data_args.voxel_margin
    ...
    model.fast3r.to(f"cuda:{local_rank}")
    model.encoder_to_decoder_proj.to(f"cuda:{local_rank}")
    model.st_attention_block.to(f"cuda:{local_rank}")
    model.mlp_pe.to(f"cuda:{local_rank}")
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.lora_enable:
            N = 4  # unforzen last 4 layers
            total_layers = model.base_model.model.model.layers
            for i, layer in enumerate(total_layers):
                if i >= len(total_layers) - N:
                    for param in layer.parameters():
                        param.requires_grad = True
            for name, param in model.named_parameters():
                if "embed_tokens" in name or "lora_" in name or "ground_head" in name or "mm_projector" in name or "encoder_to_decoder_proj" in name or "mlp_pe" in name: # or 'lm_head' in name :
                    param.requires_grad_(True)
                if "st_attention_block" in name:
                    param.requires_grad_(True)
        else:
        ...
        model.config.mm_new_attn_lr = training_args.mm_new_attn_lr
        model.config.mm_encoder_to_decoder_proj_lr = training_args.mm_encoder_to_decoder_proj_lr
        model.config.mm_mlp_pe_lr = training_args.mm_mlp_pe_lr
