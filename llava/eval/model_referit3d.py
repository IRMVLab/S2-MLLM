...
from llava.model.build_config  import load_pretrained_model
...
def eval_model(questions, args):
    ...
    model_name = "llavanext-qwen-lora-uniform"
    config = {}
    ...
    for line in tqdm(questions):
        ...
        for k, v in video_dict.items():
            if isinstance(v,int):
                ...
                video_dict_bf16[k] = v.to(dtype=torch.bfloat16)
        image_tensors = video_dict_bf16.pop('images').to(model.device)
        for k in video_dict_bf16:
            video_dict_bf16[k] = video_dict_bf16[k].to(model.device)
        with torch.inference_mode():
            ...
            try:
                ...
                pred_box = video_dict_bf16['objects'][0][pred_id].tolist()
            except:
        ...
        with file_lock:
            item = {
                ...
                "view_dep": line["metadata"]['view_dep'],
            }
            ret.append(item)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bbox-type", type=str, default=None)
    ...
    if os.path.exists(args.answer_file):
        ...
        exit()
    os.environ["RAY_OBJECT_SPILLING_CONFIG"] = '{"type":"filesystem","params":{"directory_path":"ray_spill"}}'
