...
class Regr3DMultiview(Criterion, MultiLoss):
    ...
    def depthmap_to_pts3d(self,depth, pseudo_focal, pp=None, **_):
        """
        Args:
        ...
        grid_x, grid_y = self.xy_grid(W, H, cat_dim=0, device=depth.device)[:, None]
    ...
    def get_pts3d(self,gt_anchor, raw=False):
        """res = self.depth_to_pts3d()
        if not raw:
            res = [dm[: h * w].view(h, w, 3) for dm, (h, w) in zip(res, self.imshapes)]
        return res"""
        """
        views["camera_intrinsics"] = camera_intrinsics
        views["depths"] = depths
        views["camera_pose"] = camera_pose
        """
        depth = gt_anchor['depths']
        intrinsics = gt_anchor['camera_intrinsics']  
        cam2world = gt_anchor['camera_pose']
        pts3d = self.depthmap_to_pts3d(depth, intrinsics, cam2world=cam2world)
        return pts3d
    
    def compute_loss(self, gts, preds, **kw):
        gts = gts[0]
        depth = [gts['depths'][i].unsqueeze(0) for i in range(gts['depths'].shape[0])]
        intrinsics = [gts['intrinsics'][i].unsqueeze(0) for i in range(gts['intrinsics'].shape[0])]
        cam2world = [gts['cam2world'][i].unsqueeze(0) for i in range(gts['camera_pose'].shape[0])]

        gts['pts3d'] = []
        gts['valid_mask'] = []
        for i in range(len(depth)):
            pts3d,valid_mask = depthmap_to_absolute_camera_coordinates(depth,intrinsics,cam2world)
            gts['pts3d'].append(pts3d)
            gts['valid_mask'].append(valid_mask)
        ...
        details = {}
        monitoring = {}
        #gt_anchor = self.get_pts3d(gt_anchor)
...
class Regr3DMultiviewV4(Criterion, MultiLoss):
    ...
    def get_pts3d_from_views(self, gt_views, pred_views, dist_clip=None, local=False):
        ...
        for gt_view, pred_view in zip(gt_views, pred_views):
            if local:
                ...
                gt_pts = geotrf(np.float32(inv_matrix_anchor.to(torch.float32).detach().cpu()), gt_view["pts3d"])  # Transform GT points to anchor view
                pr_pts = pred_view.get("pts3d_in_other_view")  # Predicted points in anchor view
            ...
            gt_pts_list.append(torch.tensor(gt_pts).to(device = 'cuda'))
            pr_pts_list.append(pr_pts.to(device = 'cuda'))
            valid_mask_list.append(valid_gt)
    ...
    def compute_loss(self, gts, preds, **kw):
        ...
        gts = gts[0]
        depth = gts['depths']
        cam2world = gts['camera_pose']
        gts['pts3d'] = []
        gts['valid_mask'] = []
        num_views,_,_ = depth.shape
        for i in range(num_views):
            pts3d,valid_mask = depthmap_to_absolute_camera_coordinates(depth[i],gts['camera_intrinsics'],cam2world[i])
            view_dict = {}
            view_dict['pts3d'] = torch.tensor(pts3d).unsqueeze(0)
            view_dict['valid_mask'] = torch.tensor(valid_mask).unsqueeze(0)
            view_dict['depths'] = depth[i].unsqueeze(0)
            view_dict['camera_pose'] = cam2world[i].unsqueeze(0)
            view_dict['camera_intrinsics'] = gts['camera_intrinsics']
            gt_list.append(view_dict)
        
        total_loss = []
        details = {}
        self_name = "Regr3DMultiviewV3"
        
        gt_pts_list, pr_pts_list, valid_mask_list = self.get_pts3d_from_views(gt_list, preds, **kw)
        ...
        for i, (gt_pts, pr_pts, valid_mask) in enumerate(zip(gt_pts_list, pr_pts_list, valid_mask_list)):
            ...
            pr_pts_flat = pr_pts.reshape(-1, 3)     # shape: (378*378, 3)
            mask_flat = valid_mask.reshape(-1) 
            gt_pts_flat = gt_pts.reshape(-1, 3) 
            loss = self.criterion(pr_pts_flat[mask_flat], gt_pts_flat[mask_flat])
...
class ConfLossMultiviewV2(MultiLoss):
    ...
    def compute_loss(self, gts, preds, **kw):
        ...
        """for i in range(len(preds)):
            for key,value in preds[i].items():
                preds[i][key] = value.squeeze(0)"""
        total_loss, details = self.pixel_loss(gts, preds, **kw)
        ...
        for loss, mask, loss_type in total_loss:
            if loss_type == "global":
                conf_key = "conf"
                test_tensor = preds[global_count][conf_key] 
                conf, log_conf = self.get_conf_log(preds[global_count][conf_key][mask].to(device = loss.device))
