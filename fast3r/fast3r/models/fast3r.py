...
class Fast3R(nn.Module,
    ...
    def set_freeze(self, freeze):  # this is for use by downstream models
        ...
        to_be_frozen = {
            ...
            "all": [self.downstream_head, self.decoder,self.head],
        }
    ...
    def forward(self, encoded_feats, positions, shapes, profiling=False):
        """
        Args:
        ...
        """for i in range(B):
            num_patches = N  # Get the number of patches for this image
            # Extend the image_ids list with the current image ID repeated num_patches times
            image_ids.extend([i] * num_patches)"""
...
class Fast3RDecoder(nn.Module):
    ...
    def forward(self, encoded_feats, positions, image_ids):
        ...
        #print("--decode input--",x.shape,pos.shape) # [1, 729, 1024]) torch.Size([1, 11664, 2])
        #print(x.shape, pos.shape)  #torch.Size([1, 729, 1024]) torch.Size([1, 11664, 2])
        final_output = [x]  # before projection

        # project to decoder dim
        x = self.decoder_embed(x)
        #print("--decode project--",x.shape)  # ([1, 729, 1024])
